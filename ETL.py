...
def determine_ma_chuyen_nganh(lop: str) -> tuple:
    ...
    - Có CTS: tạo mới Ngành NULL_CTS và Chuyên ngành NULL_CTS, gán cho TĐHKT
    - Có QT: tạo mới Ngành NULL_QT và Chuyên ngành NULL_QT, gán cho PĐT
...
def transform_with_nlp_long_format(df_raw: pd.DataFrame) -> tuple:
    ...
    # ===== 2. XỬ LÝ CÂU TRẮC NGHIỆM - TẠO ĐỦ 12 CÂU =====
    mcq_df = df_raw[
    ...
    if not mcq_df.empty:
        ...
        # TẠO ĐỦ 12 CÂU HỎI CHO MỖI SUBMISSION
        print(f"  -> Đang xử lý câu trắc nghiệm, đảm bảo đủ 12 câu...")
        
        # Lấy tất cả submission IDs
        all_submissions = mcq_df['SubmissionID'].unique()
        
        # Tạo DataFrame đầy đủ với 12 câu hỏi cho mỗi submission
        complete_data = []
        missing_count = 0
        ...
        for sub_id in all_submissions:
            ...
            if len(existing_questions) >= 12:
                # Chỉ lấy 12 câu có điểm cao nhất nếu có nhiều hơn 12
                if len(existing_questions) > 12:
                    # Lấy câu hỏi có điểm cao nhất
                    top_questions = sub_data.nlargest(12, 'Diem')[['SubmissionID', 'MaCauHoi', 'Diem']]
                    for _, row in top_questions.iterrows():
                        complete_data.append((row['SubmissionID'], row['MaCauHoi'], row['Diem']))
                else:
                    for _, row in sub_data.iterrows():
                        complete_data.append((row['SubmissionID'], row['MaCauHoi'], row['Diem']))
            else:
                # Thiếu câu hỏi, thêm vào với điểm cao nhất có thể (5)
                missing_count += 1
                # Thêm các câu đã có
                for _, row in sub_data.iterrows():
                    complete_data.append((row['SubmissionID'], row['MaCauHoi'], row['Diem']))
                
                # Thêm các câu còn thiếu với điểm 5 (cao nhất)
                missing_questions = set(range(1, 13)) - existing_questions
                for q in missing_questions:
                    complete_data.append((sub_id, q, 5))
        
        fact_ketqua = pd.DataFrame(complete_data, columns=['SubmissionID', 'MaCauHoi', 'Diem'])
        
        if missing_count > 0:
            print(f"  ⚠️ Có {missing_count} submissions thiếu câu hỏi, đã thêm với điểm 5")
        
        print(f"  -> FACT_KET_QUA_DANH_GIA: {len(fact_ketqua):,} dòng ({len(fact_ketqua)/12:.0f} submissions x 12 câu)")
    else:
...
def load_dimensions_optimized(cursor, df_raw, hp_master, dim_nganh, dim_chuyennganh, mapping):
    ...
    # Thêm các ngành đặc biệt cho CTS và QT
    default_nganh = [
    ...
    # ==========================================
    # BẢNG 3: DIM_CHUYEN_NGANH (INSERT SAU KHI ĐÃ COMMIT)
    # ==========================================
    print("\n  -> 3. DIM_CHUYEN_NGANH")
    cursor.execute("SELECT MaNganh FROM DIM_NGANH")
    ...
    # Thêm các chuyên ngành đặc biệt cho CTS và QT
    default_chuyennganh = [
    ...
    for ma_cn, ten_cn, ma_nganh in default_chuyennganh:
        if ma_cn not in existing_cn:
            cursor.execute("""
        ...
        for _, row in dim_chuyennganh.iterrows():
            ...
            if ma_chuyen and ma_chuyen not in existing_cn:
                ...
                if ma_nganh not in valid_nganh:
                    ...
                    continue
                data_cn.append((ma_chuyen, row['TenChuyenNganh'], ma_nganh))
                existing_cn.add(ma_chuyen)
        
        if data_cn:
            batch_insert_optimized(cursor, 'DIM_CHUYEN_NGANH', 
                                  ['MaChuyenNganh', 'TenChuyenNganh', 'MaNganh'], data_cn, 5000)
    ...
    # ==========================================
    # BẢNG 7: DIM_LOP_SINH_VIEN (DEBUG)
    # ==========================================
    print("\n  -> 7. DIM_LOP_SINH_VIEN")
...
def main():
    ...
    print(f"   - Số submissions trắc nghiệm: {count_kq // 12 if count_kq > 0 else 0:,}")
