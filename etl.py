...
def split_after_null_by_scoring(after_null_list):
    ...
    # Level 1
    parts1 = split_by_condition_1(original_text)
    if len(parts1) == 4: return parts1[:4]
    if len(parts1) == 3:
        success, new_parts = try_create_4th_column(parts1)
        if success: return new_parts[:4]
    
    # Level 2
    parts2 = split_by_condition_2(original_text)
    if len(parts2) == 4: return parts2[:4]
    if len(parts2) == 3:
        success, new_parts = try_create_4th_column(parts2)
        if success: return new_parts[:4]
    
    # Level 3
    parts3 = split_by_condition_3(original_text)
    if len(parts3) == 4: return parts3[:4]
    if len(parts3) == 3:
        success, new_parts = try_create_4th_column(parts3)
        if success: return new_parts[:4]
    
    # Fallback
    best_parts = parts3 if len(parts3) >= len(parts2) else parts2
    best_parts = best_parts if len(best_parts) >= len(parts1) else parts1
    ...
    for size in sizes:
        ...
        result.append(', '.join(best_parts[start:end]))
        start = end
    return result
...
def is_ma_gv_format(value):
    return isinstance(value, str) and bool(_ma_gv_pattern.match(value.strip()))
...
def determine_ma_chuyen_nganh(lop: str) -> tuple:
    ...
    if _lop_pattern.match(lop_normalized):
        return f"K{lop_normalized[3:5]}", None, None, is_cts
    if is_cts:
